...
# 0 is usually the default. Change to 1, 2, or 3 if you have multiple cameras plugged in.
CAMERA_INDEX = 0  
...
# ==========================================
def vision_loop():
    global latest_dx, latest_dy, latest_dz, latest_droll, latest_dpitch, latest_dyaw, latest_grip, is_running
    
    print(f"[Brain] Starting Logitech Camera (Index {CAMERA_INDEX}) and AI...")
    
    # Initialize OpenCV Video Capture
    cap = cv2.VideoCapture(CAMERA_INDEX)
    if not cap.isOpened():
        print(f"[Brain] ERROR: Could not open camera {CAMERA_INDEX}. Try changing CAMERA_INDEX.")
        is_running = False
        return
    ...
    try:
        while is_running:
            # Grab frame from Logitech camera
            ret, frame = cap.read()
            if not ret: 
                print("[Brain] Warning: Dropped a camera frame.")
                continue
            
            image_np = frame
            # OpenCV captures in BGR, PIL needs RGB
