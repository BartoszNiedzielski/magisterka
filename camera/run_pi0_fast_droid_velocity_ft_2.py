...
# Gripper settings (Franka parallel gripper)
GRIPPER_OPEN_WIDTH_M = 0.08
GRIPPER_CLOSE_WIDTH_M = 0.0
# Hysteresis to avoid flicker when model outputs mid-values
...
def vision_loop(cap_ext, cap_wrist, policy):
    ...
    try:
        while is_running:
            ...
            with state_lock:
                # Use actual finger width to derive a continuous [0,1] position signal.
                gs = gripper.read_once()
                width = float(gs.width)
                max_width = float(gs.max_width) if float(gs.max_width) > 1e-9 else GRIPPER_OPEN_WIDTH_M
                current_grip_state = float(np.clip(1.0 - (width / max_width), 0.0, 1.0))
            ...
            elif hasattr(action_chunk, "device"):
...
def control_loop():
    ...
    while is_running:
        ...
        with state_lock:
            if latest_action_chunk is not None and (current_chunk is None or step_index >= len(current_chunk)):
        ...
        if current_chunk is not None and step_index < len(current_chunk):
            ...
            try:
                ...
                if (
                    safe_target_pos[0] > 0.7
                    or safe_target_pos[0] < 0
                    or safe_target_pos[1] > 0.3
                    or safe_target_pos[1] < -0.3
                    or safe_target_pos[2] > 0.65
                    or safe_target_pos[2] < 0.06
                ):
            ...
            try:
                ...
                if grip >= GRIPPER_CLOSE_THRESH:
                    print("should be closing")
                with state_lock:
                    current_grip_state = latest_grip

                if grip >= GRIPPER_CLOSE_THRESH and current_grip_state == 0.0:
                    print("[Muscle] Closing gripper...")
                    gripper.grasp(width=GRIPPER_CLOSE_WIDTH_M, speed=0.1, force=40)
                    with state_lock:
                        latest_grip = 1.0
                elif grip <= GRIPPER_OPEN_THRESH and current_grip_state == 1.0:
                    print("[Muscle] Opening gripper...")
                    gripper.move(width=GRIPPER_OPEN_WIDTH_M, speed=0.1)
                    with state_lock:
                        latest_grip = 0.0
            except Exception:
                pass
